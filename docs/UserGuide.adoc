...
[NOTE]
====
Undoable commands: those commands that modify the finance tracker's content (`add`, `delete`, `edit` and `clear`), modify budget (`addbudget`, `deletedudget` and `editbudget`) and modify debt (`adddebt`, `deletedebt`, `editdebt` and `paydebt`).
====
