...
* Items in square brackets are optional e.g `n/NAME [r/REMARKS]` can be used as `n/Hamburger r/my fav type of junk food` or as `n/Hamburger`.
...
e.g. When adding an expense, instead of inputting an entire line of command such as `addexpense n/NAME $/AMOUNT c/CATEGORY [d/DATE] [r/REMARK]` (Format 1), we also allow user to type in command `add` and prompts the user to input each parameter one by one. (Format 2).
====
...
`Please enter the start date of budget: 01-02-2019` +
`Please enter the end date of budget: 28-02-2019`
