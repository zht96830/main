...
[NOTE]
====
Undoable commands: those commands that modify the finance tracker's content (`add`, `delete`, `edit` and `clear`), modify budget (`addBudget`, `deleteBudget` and `editBudget`) and modify debt (`addDebt`, `deleteDebt`, `editDebt` and `paidDebt`).
====
...
Restores the finance tracker to the state before multiple previous _undoable_ commands were executed. +
Format: 'undo' NUMBER
...
Restores the finance tracker to the state before a specific _undoable_ command was executed. +
Format: 'undo' INDEX
