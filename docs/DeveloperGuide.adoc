...
The Recurring feature consists of `addrecurring`, `listrecurring`, `editrecurring` and `deleterecurring`.

This feature allows users to add recurring expenses: expenses that are periodic and have a fixed frequency and a defined number of occurrences. Examples of these 'recurring' are bills or subscription fees.

Instead of manually adding the same expense repetitively, use `addrecurring` to add a Recurring. The Finance Tracker will automatically add the expense for you periodically at your specified frequency and for your specified duration!

This feature also simplifies the deletion and editing of these recurring expenses.

==== Future Implementation of Automatic expense adding

For each recurring with occurrence "O" created, create an array of size O, A[O]. Inside each index is the date that an expense has to be added due to this recurring, i.e. A recurring created with Frequency = M and Occurrence = 2 on Date = 01-01-19 will create an array A of size 2, where A[0] = 01-02-19 and A[1] = 01-03-19.

After an expense has been automatically created for each occurrence of the recurring, the specific index will be converted to null.

Upon each launch of the MainApp, a check is done against the array of every recurring, comparing the current System date against non-null indexes. Any dates earlier or equals to the current System date will mean that an expense has to be automatically created for that recurring.

==== Future Implementation of delete/edit recurring with pastOption = "Y"
...
User enters the amount they want to limit themselves to for a particular category and specifies the start and end date for their budget.
For every expense they enter that falls under that category, the totalSpent variable in the Budget class will be updated (coming in v1.3). This information will be displayed to the user so that they can know how much they have spent so far and better plan their future expenses so as not to exceed their budget.
...
[TIP]Do take a look at <<Design-Ui>> before attempting to modify the `UI` component.
...
. Modify the link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to show the total number of expense in the finance tracker.
+
**Before**
+
...
** link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] needs to initialize the status bar on application start, and to update it accordingly whenever the finance tracker is updated.
* Solution
...
** For tests, modify link:{repoURL}/src/test/java/guitests/guihandles/StatusBarFooterHandle.java[`StatusBarFooterHandle`] by adding a state-saving functionality for the total number of expense status, just like what we did for save location and sync status.
...
. Modify link:{repoURL}/src/main/java/seedu/address/logic/parser/FinanceTrackerParser.java[`FinanceTrackerParser`] to accept a `RemarkCommand`.
...
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `data/financetracker.json` so that the application will load the sample data when you launch it.)

===== [Step 6] Storage: Add `Remark` field to `JsonAdaptedExpense` class
...
. Fix `invalidAndValidExpenseFinanceTracker.json`, `typicalExpensesFinanceTracker.json`, `validFinanceTracker.json` etc., such that the JSON tests will not fail due to a missing `remark` field.
