...
4. The `calculateStatistics` method in `Statistics` is a wrapper that will call the appropriate method `basicStats(01-01-2019, 01-02-2019)` after reading the String `"stats"` that was passed in.

5.  `basicStats` calls the method `extractRelevantExpense(01-01-2019, 01-02-2019)` which takes only the `Expenses` that lie within the time range and adds them to a new `Expense` list `data`. These relevant `Expenses` are then processed by the `generateSummary(data)` method. This method will return a table of relevant statistics.

6. The methods `trimTable(table)`, `convertDataToString(trimmedtable)` are called to process this table. `htmlTableBuilder(header, table)` is called to generate the HTML code required to show the Statistics in the BrowserPanel.

7. `Model Manager` calls `setStatistics(statistics)` which sets the observable `Statistics` in `ModelManager`.

8. The listerner in `BrowserPanel` will detect the change and display the HTML page showing the statistics
...
Editing, deleting and selecting budgets work similarly to adding budgets.
